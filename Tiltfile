...
helm_remote('couchdb', repo_url='https://apache.github.io/couchdb-helm', values=['couchdb-values.yaml'])
k8s_resource(
  workload='couchdb-couchdb',
  port_forwards=5984
)
local_resource('create-system-dbs',
    cmd='curl -X PUT http://admin:this_is_not_secure@127.0.0.1:5984/_users && curl -X PUT http://admin:this_is_not_secure@127.0.0.1:5984/_replicator && curl -X PUT http://admin:this_is_not_secure@127.0.0.1:5984/_global_changes',
    resource_deps=['couchdb-couchdb'])
